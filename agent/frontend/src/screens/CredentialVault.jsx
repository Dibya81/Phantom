    ...
    <div style={{ minHeight: '100vh', background: 'var(--sys-bg)', padding: '40px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 48, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#007AFF', display: 'block', marginBottom: 12 }}>{t.vault.sub}</span>
            <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 40, fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.04em' }}>{t.vault.headline}</h1>
          </div>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>{t.vault.statusLabel}</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#10B981', boxShadow: '0 0 12px #10B981' }} />
              <span style={{ fontSize: 14, fontWeight: 700, color: '#10B981' }}>{t.vault.liveStatus}</span>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <motion.div whileHover={{ y: -4 }} style={{ padding: 32, background: 'var(--sys-panel)', border: '1px solid var(--sys-panel-border)', borderRadius: 24, backdropFilter: 'blur(16px)' }}>
              <div style={{ display: 'flex', gap: 48 }}>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 12 }}>{t.vault.pseudonymLabel}</p>
                  <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 16, color: '#007AFF' }}>{profile?.pseudonym?.slice(0, 16)}...</p>
                </div>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 12 }}>{t.vault.riskProfileLabel}</p>
                  <span style={{ 
                  ...
                  }}>{t.risk[profile?.risk_level] || profile?.risk_level}</span>
                </div>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: 12 }}>{t.vault.lastAuditLabel}</p>
                  <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>{profile?.last_scan ? new Date(profile.last_scan).toLocaleString() : t.vault.never}</p>
                </div>
              </div>
            </motion.div>
            ...
            {/* Proofs List */}
            <div>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 20 }}>{t.vault.proofsLabel}</h3>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                        ...
                        <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{c.threat_assessment?.matches[0]?.breach || t.vault.detected}</span>
                      </div>
                      <p style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: 'var(--text-muted)', marginBottom: 4 }}>
                        {t.vault.sigLabel}: {c.proof_result?.solana_tx_sig?.slice(0, 24)}...
                      </p>
                      <p style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#007AFF', opacity: 0.8 }}>
                        {t.vault.hashLabel}: {c.proof_result?.report_hash}
                      </p>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: 10, fontWeight: 800, color: '#10B981', display: 'block', marginBottom: 4 }}>{t.vault.verifiedLabel}</span>
                      <a href={`https://explorer.solana.com/tx/${c.proof_result?.solana_tx_sig}?cluster=devnet`} target="_blank" rel="noreferrer" 
                        style={{ fontSize: 11, color: '#007AFF', textDecoration: 'none' }}>{t.vault.solana}</a>
                    </div>
                  </motion.div>
                )) : (
                  <div style={{ padding: 48, textAlign: 'center', border: '2px dashed var(--sys-panel-border)', borderRadius: 24 }}>
                    <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>{t.vault.noCredentials}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ padding: 28, background: 'var(--sys-panel)', border: '1px solid var(--sys-panel-border)', borderRadius: 24, backdropFilter: 'blur(10px)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                ...
                <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{t.vault.fingerprintsLabel}</h3>
              </div>
              
              <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', margin: 0, letterSpacing: '0.05em' }}>{t.vault.emailHashLabel}</p>
                  <span style={{ fontSize: 9, color: '#007AFF', fontWeight: 800 }}>{t.vault.protectedLabel}</span>
                </div>
                <motion.div 
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', margin: 0, letterSpacing: '0.05em' }}>{t.vault.phoneHashLabel}</p>
                  <span style={{ fontSize: 9, color: '#007AFF', fontWeight: 800 }}>{t.vault.protectedLabel}</span>
                </div>
                <motion.div 
                   whileHover={{ background: 'rgba(0,122,255,0.05)', borderColor: 'rgba(0,122,255,0.2)' }}
            ...
            <div style={{ padding: 28, background: 'linear-gradient(135deg, #007AFF, #0044FF)', borderRadius: 24, color: '#fff' }}>
              <p style={{ fontSize: 12, fontWeight: 700, opacity: 0.8, marginBottom: 8 }}>{t.vault.totalExposuresLabel}</p>
              ...
              <p style={{ fontSize: 13, marginTop: 16, opacity: 0.9, lineHeight: 1.5 }}>
                {t.vault.exposuresSummary.replace('{count}', profile?.breach_count || 0)}
              </p>
            </div>

            <div style={{ padding: 24, background: 'var(--sys-panel)', border: '1px solid var(--sys-panel-border)', borderRadius: 24 }}>
              <h3 style={{ fontSize: 11, fontWeight: 800, color: '#007AFF', letterSpacing: '0.15em', marginBottom: 16 }}>{t.vault.networkActivityLabel}</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {profile?.recent_global_scans?.map((scan, i) => (
                  <div key={i} style={{ padding: '10px 12px', background: 'rgba(0,0,0,0.15)', borderRadius: 8, borderLeft: '3px solid #007AFF' }}>
                    <p style={{ fontSize: 9, color: 'var(--text-muted)', margin: '0 0 4px', fontWeight: 700 }}>{t.vault.identityHashLabel}</p>
                ...
                {!profile?.recent_global_scans && (
                   <p style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>{t.vault.syncingLabel}</p>
                )}
              </div>
            </div>
